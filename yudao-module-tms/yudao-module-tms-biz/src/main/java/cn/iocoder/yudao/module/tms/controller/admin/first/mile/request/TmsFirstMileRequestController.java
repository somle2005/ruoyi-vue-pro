...
public class TmsFirstMileRequestController {
    ...
    public CommonResult<Boolean> updateFirstMileRequest(@Valid @RequestBody TmsFirstMileRequestSaveReqVO updateReqVO) {
    ...
    @PreAuthorize("@ss.hasPermission('tms:first-mile-request:query')")
    public CommonResult<PageResult<TmsFirstMileRequestRespVO>> getFirstMileRequestPage(@Valid TmsFirstMileRequestPageReqVO pageReqVO) {
    ...
    public void exportFirstMileRequestExcel(@Valid TmsFirstMileRequestPageReqVO pageReqVO,
                                            HttpServletResponse response) throws IOException {
        pageReqVO.setPageSize(PageParam.PAGE_SIZE_NONE);
        List<TmsFirstMileRequestDO> list = firstMileRequestService.getFirstMileRequestPage(pageReqVO).getList();
        // 导出 Excel
        ExcelUtils.write(response, "头程申请单.xls", "数据", TmsFirstMileRequestRespVO.class,
            BeanUtils.toBean(list, TmsFirstMileRequestRespVO.class));
    }
    ...
    public CommonResult
        ...
        List
            <TmsFirstMileRequestSaveReqVO> list =
            ExcelUtils.read(file, TmsFirstMileRequestSaveReqVO.class);
        // 可根据业务需要批量保存或校验
        return success(true);
    }
