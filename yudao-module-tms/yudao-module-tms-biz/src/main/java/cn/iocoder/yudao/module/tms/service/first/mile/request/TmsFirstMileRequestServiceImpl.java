...
public class TmsFirstMileRequestServiceImpl implements TmsFirstMileRequestService {
    ...
    public Long createFirstMileRequest(TmsFirstMileRequestSaveReqVO createReqVO) {
        // 插入
        TmsFirstMileRequestDO firstMileRequest = BeanUtils.toBean(createReqVO, TmsFirstMileRequestDO.class);
        firstMileRequestMapper.insert(firstMileRequest);

        // 插入子表
        createFirstMileRequestItemList(firstMileRequest.getId(), BeanUtils.toBean(createReqVO.getFirstMileRequestItems(), TmsFirstMileRequestItemDO.class));
        // 返回
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void updateFirstMileRequest(TmsFirstMileRequestSaveReqVO updateReqVO) {
        // 校验存在
        validateFirstMileRequestExists(updateReqVO.getId());
        // 更新
        TmsFirstMileRequestDO updateObj = BeanUtils.toBean(updateReqVO, TmsFirstMileRequestDO.class);
        firstMileRequestMapper.updateById(updateObj);

        // 更新子表
        updateFirstMileRequestItemList(updateReqVO.getId(), BeanUtils.toBean(updateReqVO.getFirstMileRequestItems(), TmsFirstMileRequestItemDO.class));
    }
    ...
    @Override
    public PageResult<TmsFirstMileRequestItemBO> getFirstMileRequestPageBO(TmsFirstMileRequestPageReqVO pageReqVO) {
        return firstMileRequestItemMapper.selectPageBO(pageReqVO);
    }
