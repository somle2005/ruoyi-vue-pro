...
public class ColumnsItem {
	...
	@OpField(required = false , desc = "续重价格(单位:元) 按重量计价必填 0.00-30.00之间的小数，小数点后两位", example= "11")
	...
	@OpField(required = false , desc = "最小商品金额限制条件", example= "1")
	...
	@OpField(required = false , desc = "是否是默认规则：1默认，0非默认", example= "1")
	...
	@OpField(required = false , desc = "续重(单位:kg) 按重量计价必填 0.1-999.9之间的小数，小数点后一位", example= "1")
	...
	@OpField(required = false , desc = "最大商品金额限制条件", example= "1")
	...
	@OpField(required = false , desc = "运费规则类型，同template.rule_type字段", example= "2")
	private Long ruleType;

	@SerializedName("id")
	@OpField(required = false , desc = "id", example= "1")
	private Long id;
