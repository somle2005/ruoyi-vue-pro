...
public class ErpProductCategoryServiceImpl implements ErpProductCategoryService {
    ...
    public Long createProductCategory(ErpProductCategorySaveReqVO createReqVO) {
        ...
        ThrowUtil.ifSqlThrow(erpProductCategoryMapper.insert(category), DB_INSERT_ERROR);
        //将自定义字段保存到中间表中
        List<ErpProductCategoryFieldSaveReqVO> fields = createReqVO.getFields();
        if (CollUtil.isNotEmpty(fields)){
            List<ErpProductCategoryFieldDO> erpProductCategoryFieldDos = CategoryFieldConvert.INSTANCE.convertList(fields);
            //批量存储
            ThrowUtil.ifThrow(erpProductCategoryFieldMapper.insertBatch(erpProductCategoryFieldDos),DB_INSERT_ERROR);
        }
        // 返回
    ...
    public void updateProductCategory(ErpProductCategorySaveReqVO updateReqVO) {
        ...
        if (!Objects.equals(oldStatus,newStatus)){
            ...
            if (CollUtil.isNotEmpty(childrenList)){
                //修改所有的子孙类状态
                ThrowUtil.ifThrow(erpProductCategoryMapper.updateBatch(childrenList),DB_UPDATE_ERROR);
            }
        }
        ThrowUtil.ifSqlThrow(erpProductCategoryMapper.updateById(updateObj), DB_UPDATE_ERROR);
        //获取字段列表
        List<ErpProductCategoryFieldSaveReqVO> fields = updateReqVO.getFields();
        if (CollUtil.isNotEmpty(fields)){
            List<ErpProductCategoryFieldDO> erpProductCategoryFieldDos = CategoryFieldConvert.INSTANCE.convertList(fields);
            //修改字段列
            updateCategoryFieldList(id,erpProductCategoryFieldDos);
        }
    }
    ...
    public void deleteProductCategory(Long id) {
        ...
        // 2. 删除
        ThrowUtil.ifSqlThrow(erpProductCategoryMapper.deleteById(id),DB_DELETE_ERROR);
        //3.删除中间表的数据
    ...
    * @Author Wqh
    * @Description 更新字段列表
    * @Date 16:45 2024/10/18
    * @Param [id, fields]
    **/
    private void updateCategoryFieldList(Long id,List<ErpProductCategoryFieldDO> fields) {
        // 第一步，对比新老数据，获得添加、修改、删除的列表
        List<ErpProductCategoryFieldDO> oldList = erpProductCategoryFieldMapper.selectListByCategoryId(id);
        // id 不同，就认为是不同的记录
        List<List<ErpProductCategoryFieldDO>> diffList = diffList(oldList, fields,
                (oldVal, newVal) -> oldVal.getId().equals(newVal.getId()));

        // 第二步，批量添加、修改、删除
        if (CollUtil.isNotEmpty(diffList.get(0))) {
            diffList.get(0).forEach(o -> o.setCategoryId(id));
            ThrowUtil.ifThrow(erpProductCategoryFieldMapper.insertBatch(diffList.get(0)),DB_UPDATE_ERROR);
        }
        if (CollUtil.isNotEmpty(diffList.get(1))) {
            ThrowUtil.ifThrow(erpProductCategoryFieldMapper.updateBatch(diffList.get(1)),DB_UPDATE_ERROR);
        }
        if (CollUtil.isNotEmpty(diffList.get(2))) {
            ThrowUtil.ifSqlThrow(erpProductCategoryFieldMapper.deleteByIds(convertList(diffList.get(2), ErpProductCategoryFieldDO::getId)),DB_UPDATE_ERROR);
        }
    }
}