...
2024/11/14 更新：当前版本实现了产品自定义字段功能，实现方式主要是采用了EAV模式，即动态扩展字段。
在数据库中有一个表`system_field`，里面有字段类型、字段名称、字段描述、字段默认值、字段是否必填、字段是否启用等字段。
产品自定义字段是根据产品分类进行关联的，所以需要先创建产品分类，然后在创建的时候需要去选择有哪些自定义字段。
同时在数据库中还有一张`system_value`表，这张表主要是记录自定义字段的值，检索方式主要是通过classId（也就是在实体中的唯一id）
和table（也就是表的名称）
