...
import com.somle.walmart.controller.vo.*;
...
public class WalmartClient {
    ...
    public WalmartAllItemsRespVO getAllItems(WalmartGetAllItemsReqVO dto) {
        ...
        RequestX request = RequestX.builder()
            ...
            .headers(tokenValues)
            .build();
        WalmartAllItemsRespVO walmartAllItemsRespVO = null;
        try {
            ...
            walmartAllItemsRespVO = JSON.parseObject(responseJson, WalmartAllItemsRespVO.class);
        } catch (Exception e) {
        ...
        return walmartAllItemsRespVO;
    }

    public WalmartSearchRespVO searchItem(WalmartSearchReqVO dto) {
        WalmartSearchRespVO walmartSearchRespVO = new WalmartSearchRespVO();
        List<WalmartSearchRespVO.ItemsDTO> itemsDTOS = new ArrayList<>();
        ...
        for (List<String> eachGtinPartition : gtinPartition) {
            ...
            RequestX request = RequestX.builder()
                ...
                .headers(tokenValues)
                .build();
            WalmartSearchRespVO walmartAllItemsResVO = null;
            try {
                Response response = webClient.newCall(WebUtils.toOkHttp(request)).execute();
                String responseJson = response.body().string();
                if (!successCode.equals(response.code())) {
                ...
                walmartAllItemsResVO = JSON.parseObject(responseJson, WalmartSearchRespVO.class);
            } catch (Exception e) {
                log.error("请求错误", e);
                throw new RuntimeException(e);
            }
            List<WalmartSearchRespVO.ItemsDTO> items = walmartAllItemsResVO.getItems();
            if (!CollectionUtils.isEmpty(items)) {
                itemsDTOS.addAll(items);
            }
        }
        walmartSearchRespVO.setItems(itemsDTOS);
        Long sleepTime = dto.getSleepTime();
        if (sleepTime != null) {
            //防止限流
            try {
    ...
    public String getReconFile(LocalDate date) {
        ...
        WalmartGetReconFileReqVO walmartGetReconFileReqVO = new WalmartGetReconFileReqVO();
        walmartGetReconFileReqVO.setReportVersion("v1");
        walmartGetReconFileReqVO.setReportDate(dateStr);
        tokenValues.put("Accept", "application/octet-stream");
        RequestX request = RequestX.builder()
            .url(walmartTokenDO.getDomain() + endpoint)
            .requestMethod(RequestX.Method.GET)
            .queryParams(walmartGetReconFileReqVO)
            .headers(tokenValues)
            .build();
        Response response = webClient.newCall(WebUtils.toOkHttp(request)).execute();
